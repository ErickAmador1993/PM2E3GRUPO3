...
namespace Examen_3.ViewModels
{
     public class BaseViewModel : INotifyPropertyChanged
    {
        ...
        private async void AddTask(object obj)
        {
            ...
            await Application.Current.MainPage.DisplayAlert("Alert", "Pago Guardado con Exito", "OK");
        }
        ...
        public async void update()
        {
            if (!string.IsNullOrEmpty(Id_Pago.ToString()))
            {
                ...
                await Application.Current.MainPage.DisplayAlert("Success", "Pago Actualizado", "OK");

            }
            else
            {
                await Application.Current.MainPage.DisplayAlert("Required", "Por Favor Coloque las Credenciales Correctamente", "OK");
            }
        }
        ...
        private async void deleteAsync1()
        {
            if (!string.IsNullOrEmpty(id_pago.ToString()))
            {
                ...
                if (person != null)
                {
                    ...
                    await Application.Current.MainPage.DisplayAlert("Success", "Pago Eliminado Correctamente", "OK");
